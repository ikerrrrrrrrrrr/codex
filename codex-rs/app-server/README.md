...
- `thread/wokeUp` — transient notification emitted immediately before an automatic turn starts after terminal or subagent work becomes ready; includes `threadId` and `source` (`"terminal"` or `"subagent"`). It is not persisted or replayed when a thread resumes.
