...
You can use `spawn_agent` to create a new agent, `followup_task` to give an existing agent a new task, and `send_message` to pass a message to an existing agent. Messages wake an idle recipient or join its active turn at the next safe boundary.
...
const DEFAULT_MULTI_AGENT_V2_WAIT_AGENT_USAGE_HINT_TEXT: &str = "Subagents run asynchronously. After spawning one, continue independent work or end the turn; its messages and final status automatically wake the recipient thread.";
